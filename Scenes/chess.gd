...
func _ready():
	...
	print("Pieces exists:", $Pieces != null)
	print("Dots exists:", $Dots != null)
	print("Turn exists:", $Turn != null)
	print("Cards exists:", $Cards != null)
	print("Ensuring HUD…")
	ensure_hud()
	update_hud_layout()
...
func ensure_hud():
	print("ensure_hud called")
	ui_layer = get_tree().get_root().get_node("Main/CanvasLayer") as CanvasLayer
	...
	hud_group.z_index = 10000
	hud_group.visible = true
	hud_group.set_anchors_preset(Control.PRESET_TOP_LEFT)
	hud_group.size = get_viewport_rect().size
	...
	# Center text for all
	for lbl in [health_label, mana_label, moves_label, score_label]:
...
func update_hud_layout():
	...
	var x_center = screen_size.x / 2.0

	var start_y = 100
	var gap = 40

	health_label.position = Vector2(x_center - 50, start_y)
	mana_label.position = Vector2(x_center - 50, start_y + gap)
	moves_label.position = Vector2(x_center - 50, start_y + gap * 2)
	score_label.position = Vector2(x_center - 50, start_y + gap * 3)
	...
	# Make icons visible beside labels
	if health_icon and health_icon.texture:
		health_icon.scale = Vector2(0.1, 0.1)
		health_icon.position = Vector2(health_label.position.x - 40, health_label.position.y)

	if mana_icon and mana_icon.texture:
		mana_icon.scale = Vector2(0.1, 0.1)
		mana_icon.position = Vector2(mana_label.position.x - 40, mana_label.position.y)
